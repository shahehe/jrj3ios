...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions {
    ...
    if (self = [super initWithFrame:rect]) {
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(POPLISTVIEW_SCREENINSET,
                                                                   (rect.size.height-200-POPLISTVIEW_HEADER_HEIGHT)/2,
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"LeveyPopListViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"Cell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    LeveyPopListViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Cell" forIndexPath:indexPath];
    cell.nameLabel.text = _options[indexPath.row][@"title"];
    cell.subLabel.text = _options[indexPath.row][@"detail"];
    
//    if (!cell)
//        cell = [[LeveyPopListViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellIdentity];
//    
...
//    rect.size = CGSizeMake(rect.size.width, _tableView.frame.size.height);
//    CGRect bgRect = CGRectInset(rect, POPLISTVIEW_SCREENINSET, -60);
...
//    CGContextSetShadowWithColor(ctx, CGSizeZero, 6., [UIColor colorWithWhite:1 alpha:.75].CGColor);
...
//    [[UIColor colorWithRed:0.3 green:0.3 blue:0.3 alpha:1.] setFill];
